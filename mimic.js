...
detector.addEventListener("onWebcamConnectSuccess", function() {
  log('#logs', " You can access Webcam");
});
...
function drawFeaturePoints(canvas, img, face) {
  ...
  // <your code here>
  ctx.strokeStyle = 'green';  // Set Blue rectangle stroke
...
function drawEmoji(canvas, img, face) {
  ...
  // <your code here>
  ctx.strokeStyle = 'red';  // Set red rectangle stroke
  ctx.font = '60px Arial';
  ...
  // <your code here>
  feature_point = face.featurePoints[10];
  face_emoji_x = feature_point.x - 10;
  face_emoji_y = feature_point.y + 10;
...
function randomEmojiCode() {
  var randIdx = Math.floor(Math.random() * (emojis.length-1));
   var newEmoji = emojis[randIdx];
...
function incrementScore() {
  score++;
  setScore(score, total);
  log('#logs', "You Did it!! Wow :) ")
}

function incrementTotal() {
  total++;
  setScore(score, total);
  if (score > total*0.6 && total == 20) {alert(" Game Over!\nYou have good acting skills!!\n Try Hollywood ");
  $('#results').html("");  // clear out results
  $("#logs").html("");  // clear out previous log
  window.location.reload(false); ;
  }
  else if (total == 20){alert("Game Over! \n You played well! \n There is scope for improvement!!");
  $('#results').html("");  // clear out results
  $("#logs").html("");  // clear out previous log
  window.location.reload(false); ;}
}
